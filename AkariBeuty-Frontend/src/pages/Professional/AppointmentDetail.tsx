...
export default function ProfessionalAppointmentDetailPage() {
  ...
  const handleStatus = async (novoStatus: keyof typeof STATUS_CODES) => {
    ...
    <section className="py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-bolt-primary-50 space-y-4">
        <div>
          ...
          <h1 className="text-xl font-semibold text-bolt-neutral-900">{appointment.clienteNome}</h1>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm text-bolt-neutral-600">
          <div>
            ...
            <p className="font-semibold">{data.toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long" })}</p>
          </div>
          <div>
            <p className="text-xs text-bolt-neutral-500">Horário</p>
            <p className="font-semibold">{data.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p>
          </div>
        </div>
        <div>

...
export default function ProfessionalAgendaPage() {
    ...
    const handleStatusChange = async (
        appointment: ProfessionalAgendaItem,
        ...
        try {
            ...
            if (axios.isAxiosError(err)) {
                ...
                setError(responseMessage ?? "Não foi possível atualizar o status do agendamento");
            } else {
    ...
    return (
        <section className="space-y-6 py-6">
                ...
                <div className="space-y-3">
                    {dayAgenda.agendamentos.map((item) => {
                        ...
                        return (
                            <div
                                key={item.id}
                                className="bg-white rounded-2xl p-4 border border-bolt-primary-50 shadow-sm"
                            >
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h3 className="text-base font-semibold text-bolt-neutral-900">
                                            {item.clienteNome}
                                        </h3>
                                    ...
                                    <div className="text-right">
                                        <span
                                            className={`text-xs font-semibold px-3 py-1 rounded-full block ${color}`}
                                        >
                                            {item.status}
                                        </span>
                                        <Link
                                ...
                                <div className="flex gap-2 mt-4 flex-wrap">
                                    ...
                                    {canTransition(normalizeStatus(item.status), "CANCELADO") && (
                                        <button
                                            type="button"
            ...
            {!loading && weekAgenda.length > 0 && (
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-bolt-primary-50">
                    <h3 className="text-base font-semibold text-bolt-neutral-900 mb-4">
                        Resumo semanal
                    </h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {weekAgenda.map((day) => (
                            <div
                                key={day.data}
                                className="border border-bolt-primary-50 rounded-2xl p-3"
                            >
                                <p className="text-xs text-bolt-neutral-500">
                                ...
                                <p className="text-2xl font-bold text-bolt-neutral-900">
